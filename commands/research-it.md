...
Then spawn **6 agents in parallel** using the Agent tool (subagent_type: Explore). Every agent receives `$PROJECT_DESC` and must verify ALL recommendations using WebSearch or kencode-search MCP (`mcp__kencode-search__searchCode` — literal/regex search across 2M+ public repos) - no training-data assumptions allowed.
...
**Agent 5 - Architecture**: Use kencode-search MCP to find how real 2026 projects of this type structure their code. Look for directory layouts, file naming conventions, and key patterns (state management, routing, data fetching, etc.). Output a concrete directory tree and list of patterns to follow.
...
1. Every recommendation MUST be verified via WebSearch or kencode-search MCP - no guessing

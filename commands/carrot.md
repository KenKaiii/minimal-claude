...
Verify this codebase against current best practices and official documentation. Spawn 8 explore agents in parallel using the Task tool (subagent_type: Explore), each focusing on one category. Each agent must VERIFY findings using kencode-search MCP (`mcp__kencode-search__searchCode` — real code samples from 2M+ public repos) or WebSearch (official docs) - no assumptions allowed.
...
**Agent 2 - Dependencies/Libraries**: Check if library APIs being used are current or deprecated. Verify against library documentation and kencode-search MCP for how modern codebases use these libraries

**Agent 3 - Language Patterns**: Identify the primary language (TypeScript, Python, Go, etc.), verify idioms and patterns are current. Use kencode-search MCP to see how modern projects write similar code
...
**Agent 6 - Testing**: Identify test framework in use, verify testing patterns match current library recommendations. Check via docs and kencode-search MCP for modern test patterns

**Agent 7 - API/Data Handling**: Review data fetching, state management, storage patterns. Verify against current patterns via kencode-search MCP and framework docs

**Agent 8 - Error Handling**: Examine error handling patterns, verify they match library documentation. Use kencode-search MCP to compare against real-world implementations
...
3. **Verify** - Check against kencode-search MCP (real code) OR WebSearch (official docs)
...
- Anything not verified via kencode-search MCP or official docs - NO
...
Source: kencode-search MCP (X repos) | URL to official docs
```
